...
void test(int iter, double (*func_std)(void), double (*func_ft)(void), std::string test)
{
	double sum = 0;
	double mean_std = 0;
	double mean_ft = 0;
	...
	std::cout << "\033[1;32mTest \033[0m" << std::left << std::setw(12) << test << std::left << std::setw(20) << "\033[1;31m - \033[0m\033[1;32mstd: \033[0m" << std::left << std::setw(10) << mean_std <<  "\033[1;32m ft: \033[0m" <<  std::left << std::setw(10)  << mean_ft << "\033[1;32m ratio: \033[0m" << mean_ft / mean_std << std::endl;
...
int main(int argc, char** argv)
{
	...
	{
		test(iter, test_vector_std, test_vector_ft, "vector");
	}
	if (cases == "vectorErase" || cases == "all")
	{
		test(iter, test_vector_erase_std, test_vector_erase_ft, "vector erase");
	}
	if (cases == "map" || cases == "all")
	{
		test(iter, test_map_std, test_map_ft, "map");
	}
	if (cases == "mapErase" || cases == "all")
	{
		test(iter, test_map_erase_std, test_map_erase_ft, "map erase");
	}
	if (cases == "mapFind" || cases == "all")
	{
		test(iter, test_map_find_std, test_map_find_ft, "map find");
	}
	if (cases == "stack" || cases == "all")
	{
		test(iter, test_stack_std, test_stack_ft, "stack");
	}
	if (cases == "set" || cases == "all")
	{
		test(iter, test_set_std, test_set_ft, "set");
	}
	if (cases == "setErase" || cases == "all")
	{
		test(iter, test_set_erase_std, test_set_erase_ft, "setErase");
	}
	return 0;	
}